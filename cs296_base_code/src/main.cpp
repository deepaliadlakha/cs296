...
float32 average_time=0.0000;
	float32 collision_time=0.0000;
	float32 velocity_time=0.0000;
	float32 position_time=0.0000;

int32 iterations_count;
void count_sim_steps(void){
	
	float32 time_step=1/settings_hz;
	 b2World* m = test->get_world();
	const b2Profile& mp = m->GetProfile();
	int32 total_steps=0;
	

	while (total_steps<iterations_count){
	m->Step(time_step, settings.velocity_iterations, settings.position_iterations);
	average_time +=mp.step;
	collision_time +=mp.collide;
	velocity_time  +=mp.solveVelocity;
	position_time  +=mp.solvePosition;
	total_steps++;
	}
}
...
/*
 ...
 double t2=tim.tv_sec+(tim.tv_usec/1000000.0);
	...
	printf("Average time per step is %5.5f ms\n", average_time/iterations_count);
	printf("Average time for collisions is %5.5f ms\n", collision_time/iterations_count);
	printf("Average time for velocity updates is %5.5f ms\n", velocity_time/iterations_count);
	printf("Average time for position updates is %5.5f ms\n", position_time/iterations_count);
	cout<<endl; 
